...
func pushProvidersSchemaToDB(providersSchema *schema.ProvidersSchema, dbConn db.DB) {
	...
	for providerName, resources := range providersSchema.ProviderSchemas {
		...
		pushSchemas(dbConn, providerID, resources.ResourceSchemas)

		// Process each data-resource type in the provider
		pushSchemas(dbConn, providerID, resources.DataSourceSchemas)
	}
}

func pushSchemas(dbConn db.DB, providerID uuid.UUID, schemas map[string]schema.SchemaRepresentation) {
