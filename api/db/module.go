...
func (db *gDB) ListTFModule(search string, limit, offset int) (result TFModules, count int64, err error) {
	q := db.g().Model(&TFModule{})
	if search != "" {
		q = q.Where("source ILIKE '?' OR module_name ILIKE '?'", search, search)
	}
	...
	err = q.Limit(limit).Offset(offset).Order("source ASC, version DESC").Find(&result).Error
	return
}

// FindOutputMappingsByModuleID fetch the terraform module(s) by id along with it's input-attribute and the mappings to another module's outputs.
